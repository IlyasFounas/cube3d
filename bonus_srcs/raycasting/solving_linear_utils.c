...
/*   Updated: 2025/10/09 17:12:31 by ifounas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_linear_equa *linear_ray)
{
	if (linear_ray->shortest_t == linear_ray->t_vertical)
	{
		...
		if ((int)linear_ray->iy == 5 && (int)linear_ray->x == 0)
			infos->actual_orientation = PORTAL;
	...
	else
	{
		infos->tex_x = (linear_ray->ix - floor(linear_ray->ix))
			* infos->textures->t_width;
		if ((int)linear_ray->ix == 5 && (int)linear_ray->y == 0)
			infos->actual_orientation = PORTAL;
