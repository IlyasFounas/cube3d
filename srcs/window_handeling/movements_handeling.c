...
/*   Updated: 2025/10/25 14:36:36 by ifounas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_player_position(t_global_infos *infos)
{
	...
	if (keys->W == 1)
	{
		if (wall_collision(infos, infos->py, infos->px + ray->dir_x * speed) == 1)
			infos->px += ray->dir_x * speed;
		if (wall_collision(infos, infos->py + ray->dir_y * speed, infos->px) == 1)
